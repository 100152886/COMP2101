...
#function to execute when QUIT detected
function userQuit {
    echo "    !!!!    QUIT DETECTED    !!!!       "
    echo "You found the secret way to exit the script!"
    exit
}

#function to execute when INTERRUPT detected
function userInterrupt {
    echo "    !!!!    INTERRUPT DETECTED    !!!!       "
    echo "Please do not interrupt the count! Resetting."
...
#trap user commands for QUIT (ctrl+\) and INTERRUPT (ctrl+c)

trap userQuit SIGQUIT               #exectues userQuit function on QUIT signal
trap userInterrupt SIGINT           #executes userInterrupt function on INT signal
#trap sleepCount=$numberOfSleeps SIGINT
